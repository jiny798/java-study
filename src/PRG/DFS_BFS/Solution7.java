...
class Solution7 {
    ...
    public int solution(int[][] game_board, int[][] table) {
        ...
        for(int k=0;k<4;k++){
            ...
            for(int i=0;i<height;i++){
                for(int j=0;j<width;j++){
                    if(temp_arr[i][j]==0 && checked[i][j]==false) { // 0 비어있는 칸이면 조사
                        log = "";
                        gameBoardDfs(i, j, temp_arr, checked);
                        if (map.get(log) == null || map.get(log).size()==0) { //조사한 퍼즐로그가 table에 있는지 확인
                            continue;
                        } else {
                            System.out.println("합산"+log);
                            List<Integer> list = map.get(log);
                            answer += list.get(0); //있으면 카운트 합산
                            list.remove(0);
